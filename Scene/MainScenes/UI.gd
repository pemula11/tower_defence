...
func set_tower_preview(tower_type, mouse_position):
	var drag_tower = load("res://Scene/Turrent/" + tower_type + ".tscn").instance()
	...
	var texture = load("res://Asset/UI/range_overlay.png")
...
func update_health_bar(base_health):
	#hp_bar_tween.interpolate_property(node, parameter,start_value,end_value,duration,transition, easing)
	hp_bar_tween.interpolate_property(hp_bar, 'value', hp_bar.value, base_health, 0.1, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	hp_bar_tween.start()
	if base_health >= 60:
		hp_bar.set_tint_progress("0ef602")
